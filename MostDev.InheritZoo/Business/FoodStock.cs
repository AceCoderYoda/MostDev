...
// Solution      ::    MostDev
// Filename      ::    MostDev.InheritZoo.FoodStock.cs
// By            ::    Arjan Crielaard
...
using System.Linq;
using MostDev.InheritZoo.Domain.Food;
using MostDev.InheritZoo.Domain.Food.Interfaces;
using MostDev.InheritZoo.Domain.Food.Meats;
using MostDev.InheritZoo.Domain.Food.Plants;
...
namespace MostDev.InheritZoo.Business
{
    public class FoodStock
    {
        ...
        {
            if (availableFood.Sum(x => x.Amount) < requiredAmountOfFood) return false;
        ...
        public void PrintInfo()
        {
            ...
            foreach (var typeOfAnimal in types)
                Console.WriteLine(
                    $"Food Type: {typeOfAnimal.Name}, # of food: {_foodStock.Where(x => x.GetType() == typeOfAnimal).Sum(x => x.Amount)}");
        }
    }
}